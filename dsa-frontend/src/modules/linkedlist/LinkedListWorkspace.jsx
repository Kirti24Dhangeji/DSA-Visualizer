...
import { linkedListOperations } from './config/operationsConfig.js'
import { executeLinkedListOperations } from './api/linkedListApi.js'
...
export default function LinkedListWorkspace({ onBack }) {
  ...
  async function handleExecute() {
    ...
    setIsLoading(true)
    try {
      const response = await executeLinkedListOperations(queue)
      setTrace(response)
      player.reset(true)
    } catch (err) {
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-6 px-4 py-10 sm:px-8">
      <WorkspaceHeader moduleNumber={2} moduleGroup="custom-collections" title="JLinkedList" />
      ...
      <OperationPicker operations={linkedListOperations} onQueueOperation={addToQueue} />
