...
import { treeMapOperations } from './config/operationsConfig.js'
import { executeTreeMapOperations } from './api/treeMapApi.js'
...
export default function TreeMapWorkspace({ onBack }) {
  ...
  async function handleExecute() {
    ...
    setIsLoading(true)
    try {
      const response = await executeTreeMapOperations(queue)
      setTrace(response)
      player.reset(true)
    } catch (err) {
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-6 px-4 py-10 sm:px-8">
      <WorkspaceHeader moduleNumber={6} moduleGroup="custom-collections" title="JTreeMap" />
      ...
      <OperationPicker operations={treeMapOperations} onQueueOperation={addToQueue} />
