...
import { queueOperations } from './config/operationsConfig.js'
import { executeQueueOperations } from './api/queueApi.js'
...
export default function QueueWorkspace({ onBack }) {
  ...
  async function handleExecute() {
    ...
    setIsLoading(true)
    try {
      const response = await executeQueueOperations(queue)
      setTrace(response)
      player.reset(true)
    } catch (err) {
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-6 px-4 py-10 sm:px-8">
      <WorkspaceHeader moduleNumber={4} moduleGroup="custom-collections" title="JQueue" />
      ...
      <OperationPicker operations={queueOperations} onQueueOperation={addToQueue} />
