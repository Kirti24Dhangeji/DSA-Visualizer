...
import { hashSetOperations } from './config/operationsConfig.js'
import { executeHashSetOperations } from './api/hashSetApi.js'
...
export default function HashSetWorkspace({ onBack }) {
  ...
  const previousStepData = player.currentStep > 0 ? trace?.steps?.[player.currentStep - 1] ?? null : null
  ...
  async function handleExecute() {
    ...
    setIsLoading(true)
    try {
      const response = await executeHashSetOperations(queue)
      setTrace(response)
      player.reset(true)
    } catch (err) {
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-6 px-4 py-10 sm:px-8">
      <WorkspaceHeader moduleNumber={5} moduleGroup="custom-collections" title="JHashSet" />

      <OutputWindow step={currentStepData} previousStep={previousStepData} />
      ...
      <OperationPicker operations={hashSetOperations} onQueueOperation={addToQueue} />
